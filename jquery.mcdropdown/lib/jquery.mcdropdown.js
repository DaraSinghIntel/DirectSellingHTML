 ...
 */
	...
	browser.isIE = ((/\sTrident\/\d/i).test(window.navigator.userAgent));
	...
	$.mcDropDownMenu = function(el, list, options){
		...
		$divInput = $('<div class="mcdropdown"><a href="#" tabindex="-1"></a><input type="hidden" name="' + (el.name || el.id) + '" id="' + (el.id || el.name) + '" /></div>')
			.appendTo($('<div class="mcdropdown-outer" style="position: relative;"></div>'))
			.parent();
			
		// get a reference to the input element and remove it from the DOM				
		var $input = $self.replaceWith($divInput).attr({id: el.id+"-org", name: ""});
		...
		if($input.attr('data-put-label')==='yes'){
			console.log(el.id,$('label[for="'+el.id+'"]'));
			$input.after($('label[for="'+el.id+'"]'));
		}
		...
		if( browser.isIE ||(window.navigator.userAgent.indexOf('Safari') != -1 && window.navigator.userAgent.indexOf('Chrome') == -1)) $self.bind("keydown", function (e){
		...
		this.openMenu = function (e){
			...
			function open(){
				//add custom class on $divInput.parent()
				$divInput.parent().addClass('open');
		...
		this.closeMenu = function (e){
			//remove custom class on $divInput.parent()
			$divInput.parent().removeClass('open');
